...
'''
Note: This version of the helper functions compilation is a bit more polished
'''
...
# --------------------------------------------------------------------------- #
def evaluate_model_with_cm_params(model, test_loader, device=device):
    """
...
# --------------------------------------------------------------------------- #
def list_test_images(root_dir, class_names):
    """
...
# --------------------------------------------------------------------------- #
def show_image_prediction(img_path, true_label=None, model=None, preprocess=None, class_names=None, device=device):
    """
...
def plot_training_curves_seaborn(
    df, x_col="Epoch", loss_col="Train Loss", acc_col="Train Accuracy"
...
def plot_training_curves(df, x_col="Epoch", loss_col="Train Loss", acc_col="Train Accuracy", title="Training Loss & Accuracy over Epochs"):
    """
    Plots training loss and accuracy curves using Plotly Express.
    """
    fig = px.line(df, x=x_col, y=loss_col, height=600, width=800, title=title, markers=True)
    fig.add_scatter(x=df[x_col], y=df[acc_col], mode="lines+markers", name=acc_col, yaxis="y2", line=dict(width=3))
    fig.update_layout(
        template="seaborn", title_x=0.5, title_font_size=18, font=dict(size=14),
        legend=dict(title=None, orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5),
        xaxis=dict(title=x_col),
        yaxis=dict(title="Loss", titlefont=dict(color="red"), tickfont=dict(color="red")),
        yaxis2=dict(title="Accuracy (%)", titlefont=dict(color="blue"), tickfont=dict(color="blue"), overlaying="y", side="right"),
        hovermode="x unified"
    )
